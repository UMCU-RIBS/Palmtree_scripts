...
import logging
import struct
from misc import allocate_array
...
def __read_packages(file, header, file_type, read_data):
    ...
    if read_data:
        ...
        data = allocate_array((header['total_samples'], header['num_streams'] + 2))
        if data is None:
            return False, None
        ...
        if file_type == 1 and header['max_samples_stream'] > 1:
            ...
            while stream_index < header['num_streams'] and file.tell() + 4 <= header['file_size']:
                ...
                if file.tell() + (num_values * 8) <= header['file_size']:
                    ...
                    if read_data:
                        ...
                        data[row_index:row_index + num_samples, 2 + stream_index:2 + stream_index + num_streams] = \
                            np.reshape(np.fromfile(file, dtype=np.dtype('<d'), count=num_values, sep='', offset=0), (-1, num_streams))
                    ...
                    if not read_data:
                        logging.warning('Not all values in the last sample-chunk are written, discarding last sample-chunk and therefore sample-package. Stop reading.')
            ...
            if not read_data:
                ...
                if stream_index == header['num_streams']:
                    ...
                    header['total_samples'] += header['max_samples_stream']

                    # count the packages
                    header['total_packages'] += 1

                else:
                    logging.warning('Not all streams in the last sample-package are written, discarding last sample-package. Stop reading.');
                    return True, None    # consider a partial read as successful, warning is enough
                ...
                data[row_index:row_index + header['max_samples_stream'], 0] = sample_id
                data[row_index:row_index + header['max_samples_stream'], 1] = elapsed

                # move the matrix write index
                row_index += header['max_samples_stream']
            ...
            if file.tell() + (num_values * 8) <= header['file_size']:
                ...
                if read_data == 1:
                    ...
                    if num_samples == 1:
                        ...
                        data[row_index, 2:2 + header['num_streams']] = np.fromfile(file, dtype=np.dtype('<d'), count=num_values, sep='', offset=0)

                    else:

                        data[row_index:row_index + num_samples, 0] = sample_id
                        data[row_index:row_index + num_samples, 1] = elapsed
                        data[row_index:row_index + num_samples, 2:2 + header['num_streams']] = \
                            np.reshape(np.fromfile(file, dtype=np.dtype('<d'), count=num_values, sep='', offset=0), (-1, header['num_streams']))
                ...
                if not read_data:
                    logging.warning('Not all values in the last sample-package are written, discarding last sample-package. Stop reading.')
...
def load_data(filepath, read_data=False):
    ...
    try:
        ...
        with open(filepath, "rb") as file:
            ...
            if not header['version'] == 1 and not header['version'] == 2:
            ...
            if header['version'] == 1:
                ...
                if read_data:
                    ...
                    data = allocate_array((header['num_rows'], header['num_columns']))
                    if data is None:
                        return header, None
                    ...
                    for iRow in range(0, header['num_rows']):
                        ...
                        data[iRow, 1:header['num_columns']] = np.fromfile(file, dtype=np.dtype('<d'), count=header['num_columns'] - 1, sep='', offset=0)
